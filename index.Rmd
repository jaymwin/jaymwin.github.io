...
deployments_df <-
  ...
  glimpse() |>
  select(
    id, study_type, deploy_by, deploy_date, group_id, visit_id, survey_year, db_cell_id = cell_id,
    unit_number, deployment_name, swift_id, cardin, utm_zone, utme, utmn) |>
  collect() |>
  mutate(zone_utm = utm_zone, northing_utm = utmn, easting_utm = utme)
...
deployments_sf <-
  ...
  select(cell_id = db_cell_id, deployment_name, deploy_date, ownership)
...
Note that data are preliminary, and ARU deployments are jittered to keep locations confidential.

**ARU deployments**: `r total_deployments`
...
m2 <-
  ...
  mapview(layer.name = 'ARUs', col.regions = 'tomato')
